...
class SpotifySource(AudioSource):
    ...
    async def search(self, query: str, max_results: int = 5) -> List[Track]:
        ...
        try:
            if not self.client_id or not self.client_secret:
                logger.warning("Spotify credentials not configured, falling back to YouTube")
            ...
            logger.info(f"Found {len(tracks)} Spotify->YouTube tracks for query: {query}")
            return tracks
    ...
    async def get_track_info(self, url: str, requester_id: int) -> Optional[Track]:
        ...
        try:
            ...
            youtube_track = await self._find_youtube_equivalent(spotify_track, requester_id)
            if youtube_track:
    ...
    async def _find_youtube_equivalent(
        ...
        try:
            ...
            logger.warning(f"Could not find YouTube equivalent for: {artist_str} - {title}")
            return None
    ...
    def _find_best_match(
        ...
        def similarity_score(track: Track) -> float:
            ...
            # Title similarity
            title_similarity = self._text_similarity(target_title.lower(), track.title.lower())
            score += title_similarity * 0.5
            
            # Artist similarity
            if track.artist:
                artist_similarity = self._text_similarity(target_artist.lower(), track.artist.lower())
                score += artist_similarity * 0.3
            
            # Duration similarity (if both have duration)
            if target_duration > 0 and track.duration > 0:
                duration_diff = abs(target_duration - track.duration) / max(target_duration, track.duration)
                duration_score = max(0, 1 - duration_diff * 2)  # Penalize large differences
