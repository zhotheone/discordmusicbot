...
from sqlalchemy import BigInteger, String, Integer, Float, Boolean, Text, DateTime, JSON, ForeignKey
...
class User(Base):
    ...
    history: Mapped[List["UserHistory"]] = relationship("UserHistory", back_populates="user", order_by="desc(UserHistory.played_at)")
...
class GuildSettings(Base):
    ...
    default_volume: Mapped[float] = mapped_column(Float, default=0.5)
    allowed_filters: Mapped[List[str]] = mapped_column(JSON, default=lambda: ["bass_boost", "reverb", "slow", "8d"])
    command_prefix: Mapped[str] = mapped_column(String(10), default="/")
    auto_disconnect_timeout: Mapped[int] = mapped_column(Integer, default=300)  # 5 minutes
