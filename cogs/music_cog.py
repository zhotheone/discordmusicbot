...
class ConfigManager:
    ...
    def _get_path(self, guild_id: int) -> str: return os.path.join(self.config_dir, f"{guild_id}.json")
    def get_config(self, guild_id: int) -> dict:
        config_path = self._get_path(guild_id)
        if not os.path.exists(config_path):
            default_config = {"volume": 75, "active_filter": "none", "queue": []}
            self.save_config(guild_id, default_config); return default_config
        with open(config_path, 'r') as f: return json.load(f)
    def save_config(self, guild_id: int, data: dict):
        with open(self._get_path(guild_id), 'w') as f: json.dump(data, f, indent=4)

class MusicControlsView(discord.ui.View):
    def __init__(self, cog, *, timeout=None):
        super().__init__(timeout=timeout); self.cog = cog
    @discord.ui.button(label="Pause", style=discord.ButtonStyle.secondary, emoji="⏸️")
    async def pause_resume(self, interaction: discord.Interaction, button: discord.ui.Button):
        vc = interaction.guild.voice_client
        if vc and vc.is_playing(): vc.pause(); button.label = "Resume"; button.emoji = "▶️"; await interaction.response.edit_message(view=self)
        elif vc and vc.is_paused(): vc.resume(); button.label = "Pause"; button.emoji = "⏸️"; await interaction.response.edit_message(view=self)
        else: await interaction.response.defer()
    @discord.ui.button(label="Skip", style=discord.ButtonStyle.primary, emoji="⏭️")
    async def skip(self, interaction: discord.Interaction, button: discord.ui.Button):
        vc = interaction.guild.voice_client
        if vc and vc.is_playing(): vc.stop(); await interaction.response.send_message("Skipped!", ephemeral=True, delete_after=5)
        else: await interaction.response.send_message("Not playing anything to skip.", ephemeral=True)
    @discord.ui.button(label="Stop", style=discord.ButtonStyle.danger, emoji="⏹️")
    async def stop(self, interaction: discord.Interaction, button: discord.ui.Button):
        vc = interaction.guild.voice_client
        if vc and vc.is_connected():
            config = self.cog.config_manager.get_config(interaction.guild.id); config["queue"].clear()
            self.cog.config_manager.save_config(interaction.guild.id, config)
            await vc.disconnect(); await interaction.response.send_message("Playback stopped and queue cleared.", ephemeral=True, delete_after=10)
...
class MusicCog(commands.Cog):
    ...
    async def after_playback(self, interaction: discord.Interaction, error):
        if error: log.error(f"Error during playback in guild {interaction.guild.id}: {error}")
        else: log.info(f"Finished playing song in guild {interaction.guild.id}.")
    ...
    async def _play_next(self, interaction: discord.Interaction):
        ...
        if not voice_client or not voice_client.is_connected():
            log.warning(f"Play_next called in guild {guild_id} but bot is not connected."); return
        if not config.get("queue"): # Use .get() for safety
        ...
        if voice_client.is_playing():
            log.warning(f"Play_next called in guild {guild_id} but already playing something."); return
        ...
        filter_chains = {"bassboost": "bass=g=15,dynaudnorm=f=200", "nightcore": "atempo=1.3,asetrate=44100*1.3,bass=g=5", "slowedreverb": "atempo=0.8,aecho=0.8:0.9:1000:0.3"}
        if active_filter in filter_chains: ffmpeg_options['options'] += f' -af "{filter_chains[active_filter]}"'
        ...
        try:
            ...
            voice_client.play(source, after=lambda e: self.bot.loop.create_task(self.after_playback(interaction, e)))
            log.info(f"Playback started for '{song_info['title']}' in guild {guild_id}.")
        except Exception as e:
            log.exception(f"FATAL: Failed to create FFmpeg player in guild {guild_id}. THIS IS LIKELY THE PROBLEM.")
            await interaction.channel.send(embed=discord.Embed(title="Playback Error", description=f"Could not start the player. The filter might be invalid or FFmpeg is not installed correctly.\n`{e}`", color=discord.Color.red())); return
    ...
    async def on_voice_state_update(self, member, before, after):
        if member.id == self.bot.user.id and before.channel is not None and after.channel is None:
            log.info(f"Bot was disconnected from voice channel in guild {member.guild.id}. Cleaning up.")
    ...
    async def play(self, interaction: discord.Interaction, *, query: str):
        log.info(f"'{interaction.user}' in guild '{interaction.guild.name}' used /play with query: '{query}'")
        if not interaction.user.voice:
            await interaction.response.send_message("You are not connected to a voice channel.", ephemeral=True); return
        voice_channel = interaction.user.voice.channel
        voice_client = interaction.guild.voice_client
        if not voice_client:
            log.info(f"Connecting to voice channel '{voice_channel.name}' in guild {interaction.guild.id}")
            voice_client = await voice_channel.connect()
        elif voice_client.channel != voice_channel:
            log.info(f"Moving to voice channel '{voice_channel.name}' in guild {interaction.guild.id}")
        ...
        if 'entries' in info:
            ...
            log.info(f"Adding {count} songs from playlist '{info['title']}' to queue in guild {interaction.guild.id}")
            ...
            embed.title = "Playlist Added"; embed.description = f"Added **{count} songs** from `{info['title']}` to the queue."
        else:
            log.info(f"Adding song '{info.get('title')}' to queue in guild {interaction.guild.id}")
        ...
        if not voice_client.is_playing(): await self._play_next(interaction)

    async def _set_filter(self, interaction: discord.Interaction, filter_name: str, display_name: str, emoji: str):
        log.info(f"'{interaction.user}' set filter to '{filter_name}' in guild {interaction.guild.id}")
        config = self.config_manager.get_config(interaction.guild.id)
        # THIS IS THE FIX: use .get() to avoid KeyError
        if config.get("active_filter", "none") == filter_name:
            await interaction.response.send_message(f"{display_name} filter is already active.", ephemeral=True); return
        config["active_filter"] = filter_name
        self.config_manager.save_config(interaction.guild.id, config)
        embed = discord.Embed(description=f"{emoji} **{display_name}** filter activated!", color=discord.Color.gold())
        embed.set_footer(text="The effect will apply to the next song. Use /skip to apply now.")
    ...
    async def pause(self, interaction: discord.Interaction):
        ...
        if vc and vc.is_playing():
            vc.pause()
            log.info(f"Playback paused in guild {interaction.guild.id} by {interaction.user}")
    ...
    async def resume(self, interaction: discord.Interaction):
        ...
        if vc and vc.is_paused():
            vc.resume()
            log.info(f"Playback resumed in guild {interaction.guild.id} by {interaction.user}")
    ...
    async def stop(self, interaction: discord.Interaction):
        ...
        if voice_client and voice_client.is_connected():
            config = self.config_manager.get_config(interaction.guild.id); config["queue"].clear()
            self.config_manager.save_config(interaction.guild.id, config)
            await self._cleanup_now_playing(interaction.guild.id)
            await voice_client.disconnect()
            log.info(f"Playback stopped in guild {interaction.guild.id} by {interaction.user}")
    ...
    @discord.app_commands.command(name="volume", description="Sets the playback volume (0-100).")
    async def volume(self, interaction: discord.Interaction, level: int):
        log.info(f"Volume set to {level} in guild {interaction.guild.id} by {interaction.user}")
        if not 0 <= level <= 100:
            await interaction.response.send_message("Volume must be between 0 and 100.", ephemeral=True); return
        config = self.config_manager.get_config(interaction.guild.id)
        config["volume"] = level
        self.config_manager.save_config(interaction.guild.id, config)
        voice_client = interaction.guild.voice_client
        if voice_client and voice_client.source: voice_client.source.volume = level / 100.0
    ...
    async def queue(self, interaction: discord.Interaction):
        log.info(f"Queue checked in guild {interaction.guild.id} by {interaction.user}")
        config = self.config_manager.get_config(interaction.guild.id)
        ...
        await interaction.response.send_message(embed=embed)
    @discord.app_commands.command(name="nightcore", description="Applies a nightcore (speed/pitch up) effect.")
    async def nightcore(self, interaction: discord.Interaction): await self._set_filter(interaction, "nightcore", "Nightcore", "🚀")
    @discord.app_commands.command(name="slowedreverb", description="Applies a slowed + reverb effect.")
    async def slowedreverb(self, interaction: discord.Interaction): await self._set_filter(interaction, "slowedreverb", "Slowed + Reverb", "🐌")
    @discord.app_commands.command(name="bassboost", description="Applies a bass boost effect.")
    async def bassboost(self, interaction: discord.Interaction): await self._set_filter(interaction, "bassboost", "Bass Boost", "💣")
    @discord.app_commands.command(name="normal", description="Clears all audio effects.")
    async def normal(self, interaction: discord.Interaction): await self._set_filter(interaction, "none", "Normal", "✅")
